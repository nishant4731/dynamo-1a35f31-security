...
It must also accept `--force-fallback`, which selects a stricter root-resolution path that must not weaken any guarantee.
...
Only regular files/directories, symlinks, and regular-file hardlinks are supported. Before operations, audit root and descendants without following links. Reject setuid/setgid entries and block/character devices, FIFOs, Unix-domain sockets, or any other unsupported type; the verifier injects representative special nodes from image-baked fixtures. Internal regular hardlinks are valid, but their initial graph must be closed over the target: each regular inode's `st_nlink` must equal its in-target directory-entry count, even when operations do not reference it. Every symlink must have `st_nlink == 1`; hardlinked symlinks are unsupported. This unchanged-tree preflight applies in both modes and prevents alias-visible metadata writes outside the root.

The `--root` spelling is a boundary. From `/`, every existing component must be opened as a real directory and must not be followed as a link during resolution. Any symlink component rejects non-zero without changing its referenced tree; pathname shortcuts that can be raced are insufficient. Under `--force-fallback`, resolution must remain strict per-component and TOCTOU-safe. Stage the live root only through handles opened during that boundary walk, never through its pathname. A 64-run path-component swap harness alternates both modes with an 8-second bound per run: PASS requires zero outside-canary violations, exit code zero, and the modeled target tree on every synchronized attack; any canary touch, rejection, or tree mismatch fails; a safe implementation should survive indefinitely.

Publication must survive abrupt death, not merely exceptions. Fully metadata-finalize a private stage beside (not inside) the target, then publish atomically so any observer or abrupt termination sees exactly the old or complete new target—never absence, internal staging/backup entries, or mixed generations. The verifier performs ten deterministic attempts: one post-rename and four live-root mutation cuts in each CLI mode. Any absent/mixed state fails; safe publication should survive indefinitely. After abrupt termination, only target and outside canary are graded, so a private adjacent retired directory may remain.
