...
The `--root` spelling is a boundary. From `/`, every existing component must be opened as a real directory and must not be followed as a link during resolution. Any symlink component rejects non-zero without changing its referenced tree; pathname shortcuts that can be raced are insufficient. Under `--force-fallback`, resolution must remain strict per-component and TOCTOU-safe. Stage the live root only through handles opened during that boundary walk, never through its pathname. A 64-run path-component swap harness alternates both modes with a 12-second bound per run: PASS requires zero outside-canary violations, exit code zero, and the modeled target tree on every synchronized attack; any canary touch, rejection, or tree mismatch fails; a safe implementation should survive indefinitely.
